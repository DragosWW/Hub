...
const groupColorMapping = {
  'Muzică': '#007bff', 'Teatru': '#6f42c1', 'Artă': '#fd7e14',
  'Film': '#dc3545', 'Literatură': '#17a2b8',
};
...
function CalendarPage() {
  ...
  const eventPropGetter = (event, start, end, isSelected) => {
    ...
    if (event.resource && event.resource.grup) {
      const groupSlug = event.resource.grup.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
      classNames += ` event-group-${groupSlug}`;
    }
    if (event.isAdded) classNames += ' highlighted-event';
    if (isSelected) classNames += ' selected-event';
    
    // Adaugă atributul title pentru tooltip-ul default al browserului
    const tooltipText = `${event.title}\nData: ${moment(event.start).format('LLL')}\nLocație: ${event.resource.locatie || 'Nespecificată'}`;

    return { className: classNames.trim(), title: tooltipText };
  };

  // Handler pentru când se dă click pe un eveniment din calendar
  const handleSelectEvent = (calendarEvent) => {
    if (calendarEvent.resource && calendarEvent.resource.id) {
      navigate(`/eveniment/${calendarEvent.resource.id}`); // Navighează la pagina de detalii
    }
  };
  ...
  return (
    <div className="calendar-page-container">
      ...
      <Calendar
        ...
        onSelectEvent={handleSelectEvent} // ADAUGĂ ACEST PROP
        messages={{
